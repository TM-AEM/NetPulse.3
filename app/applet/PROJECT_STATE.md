...
3. **Release Keystore Missing in Container**: The release build configuration expects `my-upload-key.jks` (or a path provided via `KEYSTORE_PATH`). While `assembleDebug` builds cleanly, `assembleRelease` requires the user to supply their actual signing keystore upon Play Store export.
